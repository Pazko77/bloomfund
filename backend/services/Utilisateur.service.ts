...
import { Utilisateur, UtilisateurInput, UtilisateurOutput } from "../models/Utilisateur.model.js";
...
export const UtilisateurService = {
	...
	async create(data: UtilisateurInput): Promise<Utilisateur | null> {
		try {
			const [result] = await pool.query<ResultSetHeader>(
				...
				[data.nom, data.prenom, data.email, data.mot_de_passe, 'citoyen', data.departement]
			);
	...
	async findByEmail(email: string): Promise<Utilisateur | null> {
		try {
			const [rows] = await pool.query<RowDataPacket[]>('SELECT * FROM Utilisateurs WHERE email = ?', [email]);
	...
	async findById(id: number): Promise<Utilisateur | null> {
		try {
			const [rows] = await pool.query<RowDataPacket[]>('SELECT * FROM Utilisateurs WHERE id = ?', [id]);
			return (rows[0] as Utilisateur) || null;
		} catch (error) {
	...
	async delete(id: number): Promise<boolean> {
		try {
			const [result] = await pool.query<ResultSetHeader>('DELETE FROM Utilisateurs WHERE id = ?', [id]);
			return result.affectedRows > 0;
		} catch (error) {
	 ...
	 * @returns {Promise<Utilisateur[]>} - La liste des utilisateurs.
	 */

	async getAll(): Promise<UtilisateurOutput[]> {
		try {
			const [rows] = await pool.query<RowDataPacket[]>(
				'SELECT id, nom, prenom, email, departement, role, date_inscription FROM Utilisateurs'
			);
			return rows as UtilisateurOutput[];
		} catch (error) {
